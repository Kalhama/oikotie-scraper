...
auth_headers = {"OTA-token": None, "OTA-loaded": None, "OTA-cuid": None}
...
def get_retry(url, with_authentication):
    ...
    for i in range(1, retries):
        if (i >= 2 and with_authentication) or (
            retries == 1 and with_authentication and auth_headers["OTA-token"] == None
        ):
            authenticate()
...
def get_cards(page):
    ...
    json = get_retry(
        'https://asunnot.oikotie.fi/api/search?cardType=100&limit=24&locations=[[64,6,"Helsinki"],[39,6,"Espoo"],[65,6,"Vantaa"],[130,6,"Kauniainen"]]&offset='
        + str(offset)
...
def save_listing(card):
    ...
    r = get_retry(
        "https://asunnot.oikotie.fi/myytavat-asunnot/espoo/" + str(data["id"]), True
    )
